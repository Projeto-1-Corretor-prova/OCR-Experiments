...
class Pipeline:
    ...
    def run(self, pipeline_input: PipelineModel) -> OCRResult:
        ...
        if match(r'\w+\.'):
            raise FileExistsError("File in root folder.")
        
        pre_processed_path = pipeline_input["pre_processed_dir_path"]
        
        if not exists(pre_processed_path):
            mkdir(pre_processed_path)
        
        for student in students:
            mkdir(f"{pre_processed_path}/{student}")
